...
Графики рисуется в питоне через `matplotlib`. `subprocess` запускает скомпилированный файл и читает строку с выводом времени. Потом строит график и сохраняет в изображение (я запускал на старом wsl пока там небыло графики) 
